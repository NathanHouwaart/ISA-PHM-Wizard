...
import { GridTable, BoldCell, GrayCell } from '../GridTable/GridTable';
import { RevoGrid, Template } from '@revolist/react-datagrid';
import { flattenGridDataToMappings, getStructuredVariables } from './StudyDataTransformer';
...
export const StudyVariableSlide = forwardRef(({ onHeightChange }, ref) => {
    ...
    const gridData = useMemo(() => {
        ...
        console.log("📦 Syncing processedData → gridData", vars);
        return vars;
    ...
    useEffect(() => {
        if (!isEqual(processedData, gridData)) {
            console.log("📦 Syncing gridData → processedData");
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            ...
            setStudyVariables(processedData.map(variable => {
                return ({
                    "id": variable.id,
                    "type": variable.type,
    ...
    useEffect(() => {
        ...
        setColumns([
            { prop: 'variable', name: 'Variable', pin: 'colPinStart', size: 100, cellTemplate: Template(GrayCell), cellProperties: grayCell, },
    ...
    const handleInputChange = (variableIndex, mapping, value) => {
        setProcessedData(prevData => {
            ...
            const variable = newData[variableIndex];
            if (variable) {
                variable[mapping.studyId] = value; // Update the specific study's value     
            }
            return newData;
        });
    };
